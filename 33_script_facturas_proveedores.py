...
def LoadInvoice ():
    ...
    products = csv.reader(open('facturas_proveedor.csv', 'r'))
    header = True
    inicio = 1
    ...
    for index,row in enumerate(products):
        ...
        date_str = replace_character(str(row[4]))
        ...
        date_out_str = replace_character(str(row[5]))
        ...
        invoice.reference = row[0]
        invoice.invoice_date = fechaEmision
        invoice.type = 'in_invoice'
        invoice.save()
        invoice.no_generate_withholding = True
        ...
        if len(Product.find([('code', '=', 'PROMIGRA')])) >= 1:
            ...
            if product.template.account_category == False:
                linea.account = product.template.account_expense
            else:
                if product.template.category.account_parent == True:
                    linea.account = product.template.category.parent.account_expense
                else:
                    linea.account = product.template.category.account_expense
